...
class Controller(Node):
    ...
    def timer_callback(self):
        print(f'turtle bot msg {self.turtlebot_status_msg}') # 터틀봇 상태 : 현재 선솏도, 현재 각속도, 배터리 상태, 충전 상태 출력
        print(f'envir status {self.envir_status_msg}') # 환경 정보 : 날짜, 시간, 온도, 날씨 출력
        print(f'application status {self.app_status_msg}') # 가전 제품 : 가전상태 출력
